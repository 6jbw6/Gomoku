...
基于 Pygame-ce 原生游戏引擎构建的高性能、现代化五子棋桌面端竞技应用。支持天梯排位赛、单人休闲匹配、好友 6 位房间码对战。
...
- 天梯排位赛 (Ranked Competitive)：
  - 优先撮合同机房/校园网（局域网）内真实棋友对战，不限时检索，可随时取消；
...
- 好友房间对战 (Friend Room Code Battle)：
  ...
  - 房间码支持一键复制到剪贴板，好友经聊天工具接收后可键盘输入或直接 Ctrl+V 粘贴加入；
  - 输入框兼容中文输入法环境（键码兜底识别），汉字与全角字符自动过滤。
