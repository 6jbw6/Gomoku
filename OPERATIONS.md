...
| [`v1.3.7`](#v137---回退-v136匹配不限时输入法关联解除大写锁定问题根因修复主线程卡顿消除与对局页调亮) | 2026-09-16 | 缺陷修复 | 回退 v1.3.6、匹配不限时、stop_text_input 解除输入法关联（无需开大写）、后台预热消除卡顿、三卡统一金框、对局页调亮 | ✅ 已验证 |
...
### [v1.3.7] - 回退 v1.3.6、匹配不限时、输入法关联解除（大写锁定问题根因修复）、主线程卡顿消除与对局页调亮
- **操作日期**：2026-09-16
- **需求背景**：用户反馈“回退版本，匹配时间改为无限制，然后输入不上去是因为开大写才可以输入这是不对的。然后游戏应该都有黄圆框，匹配和好友对战没有”，随后要求“帮游戏页面调亮”。
- **根因剖析**：
  1. **开大写锁定才能输入**：中文输入法开启时，小写字母按键进入组字状态（KEYDOWN 的键码与 unicode 均被拦截）；开启 CapsLock 后输入法直通大写字母绕过组字，故“开大写才能输入”——证实根因为**游戏窗口与输入法（IME）关联**。v1.3.4 曾调用 `start_text_input()` 反而显式激活了 IME 关联。
  2. **界面卡顿致点击丢失**：点击“匹配/建房”时主线程同步执行 UDP 广播发现（约 1 秒阻塞），窗口冻结期间用户的鼠标点击丢失，输入框未获得焦点（无金色边框反馈），后续键盘输入全部无效——即用户所观察“卡顿导致输入不上”。
- **具体操作**：
  1. 回退 v1.3.6（`gomoku/gui/components.py`、`gomoku/gui/app.py`、`tests/test_gui.py` 还原至 v1.3.5 状态：移除 TEXTINPUT 通道与诊断日志）。
  2. **解除输入法关联**（`gomoku/gui/app.py`）：`pygame.key.stop_text_input()` 取代 `start_text_input()`，游戏窗口不再挂接 IME——小写字母按键直接以键码事件到达（配合 v1.3.4 键码兜底），无需开大写锁定即可输入房间码。
  3. **主线程卡顿消除**（`gomoku/gui/app.py`）：启动即后台 daemon 线程预热 `resolve_room_host()`（约 1 秒的 UDP 广播探测在后台完成并填充缓存），此后点击匹配/建房时连接走缓存零阻塞，杜绝窗口冻结与点击丢失。
  4. **匹配时间无限制**（`gomoku/gui/scenes.py`）：删除 8 秒超时停止检索与 `match_failed_at` 失败提示机制，匹配弹窗不再显示倒计时，检索持续至撮合成功或玩家主动取消。
  5. **三大模式卡片统一金色圆角边框**（`gomoku/gui/scenes.py`）：天梯排位赛、单人休闲匹配、好友房间对战三张卡片全部 `border_gold=True`，视觉规格一致。
  6. **对局页背景调亮**（`gomoku/gui/app.py`）：生成提亮版背景 `bg_surface_game`（暖色加法提亮 +34/+30/+24），对局页渲染时替代原背景，缓解全局暖炭黑遮罩的暗沉；大厅保持原有亮度。
  7. 测试修正（`tests/test_gui.py`）：本机回退用例先清理应用预热线程残留的默认端口全局单例，避免端口污染。
- **验证结果**：
  - `pytest tests -q` 28 项单元测试 **100% 全部通过**；`flake8 gomoku tests --max-line-length=100` **0 警告 0 报错**。
  - 源码与 exe 启动实测均正常。

---

### [v1.3.8] - 机房局域网撮合失联根因修复（全员自任主机队列分裂自愈）
- **操作日期**：2026-09-16
- **需求背景**：用户反馈“现在项目通过机房这个局域网匹配不了游戏了”。
- **根因剖析**：
  1. v1.3.7 为消除主线程卡顿新增启动后台预热 `resolve_room_host()`（`gomoku/gui/app.py`），机房内整机几乎同时启动，各机首轮 UDP 探测时全网尚无信标（最先自任主机的机器信标未就绪）→ **全员同时自任主机**并永久缓存 `("127.0.0.1", 8088)`；
  2. 本地回环连接恒成功、缓存永不失效，此后每次匹配都排在本机自己的服务器队列上，机房内各机队列互相隔离 → 永远撮合不到对手；且 v1.3.7 删除了 8 秒超时，检索无限空转；
  3. `discover_room_host()` 会把本机信标对定向广播/回环探测的自答当作“远端主机”，链路失效重发现时本机应答掩盖了真实远端主机。
- **具体操作**：
  1. 发现自答过滤（`gomoku/gui/room_net.py`）：新增 `discover_room_hosts()` 收集全部应答主机并过滤本机网卡 IP 与回环自答，`discover_room_host()` 改为其薄封装返回首个主机。
  2. 自任前随机退避（`gomoku/gui/room_net.py`）：`resolve_room_host()` 首轮无应答时随机退避 0.3~0.9 秒复探一次再自任主机，错开整机同时启动的竞争窗口。
  3. 收敛目标选择（`gomoku/gui/room_net.py`）：新增 `pick_converge_host()` —— 取局域网内 IP 数值最小且存活的远端主机为全网统一收敛目标；本机自任主机时仅当目标 IP 小于本机 IP 才迁移，保证唯一汇点、杜绝两台自任主机互迁抖动。
  4. 撮合空等自愈（`gomoku/gui/scenes.py`）：撮合空等 4 秒后每 5 秒后台体检收敛目标，需要时迁移重排（退出本机队列、接入收敛主机重新排队并同步地址缓存）；迁移前先轮询开局帧，恰被撮合则消息回填照常开局不迁移。
  5. 测试（`tests/test_gui.py`）：重写信标发现用例覆盖“伪造本机视角可发现/真实本机自答被过滤”两态；新增收敛目标规则（汇点留守、IP 更小才迁、失活顺延、无主机不迁）与撮合迁移-撮合成功全链路用例。
- **验证结果**：
  - `pytest tests -q` 30 项单元测试 **100% 全部通过**；`flake8 gomoku/gui/room_net.py gomoku/gui/scenes.py tests/test_gui.py` **0 警告 0 报错**。
  - 全链路模拟验证：本机自任主机排队 → 体检迁移至远端收敛主机 → 对手入队 → 双方撮合开局成功。
